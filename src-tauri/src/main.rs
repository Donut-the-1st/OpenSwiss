...
async fn add_result(state: tauri::State<'_, Mutex<Competition>>, result: String) -> Result<(), Error> {
  ...
  locked_state.prop[p1_p2] = f64::approx_from(locked_state.games_won[p1_p2]).unwrap()/f64::approx_from(locked_state.games_played[p1_p2]).unwrap();
  locked_state.prop[p2_p1] = f64::approx_from(locked_state.games_won[p2_p1]).unwrap()/f64::approx_from(locked_state.games_played[p2_p1]).unwrap();
  Ok(())
}
...
async fn get_ranks(state: tauri::State<'_, Mutex<Competition>>) -> Result<String, Error> {
  let locked_state = state.lock()?;
  let scores: Vec<no_na_nf64::NoNaNf64> = locked_state.rankings
      .to_vec()
      .iter()
      .map(|num| (no_na_nf64::NoNaNf64::new(*num)).unwrap())
      .collect();
...
fn main() {
  tauri::Builder::default()
      ...
      .invoke_handler(tauri::generate_handler![greet, add_player, initialise, add_result, update_results, get_scores, get_ranks])
